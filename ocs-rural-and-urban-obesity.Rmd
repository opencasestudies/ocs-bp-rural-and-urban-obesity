...
One of the first things that is useful to do with data is to plot the frequency of the different possible values. This is called a <b>distribution</b>. To do this we can use the `hist()` function to create a histogram.
...
```{r}
norm_BMI_ex_data <- rnorm(n = 200, mean = 24) 
...
qplot(norm_BMI_ex_data, bins = 15) 
# we can modify how we bin our histogram using the bin argument- in other words, how many bars would we like to include
```
...
```{r, eval = TRUE}
ggplot2::ggplot(BMI_long, aes(x=BMI)) + geom_histogram() 
...
```{r}
var.test(women_2017$Rural, women_1985$Rural) 
# the varaince is equal so the Wilcoxon signed rank test will work well.
wilcox.test(women_2017$Rural, women_1985$Rural, paired = TRUE)

var.test(women_2017$Urban, women_1985$Urban) 
# the varaince is equal so the Wilcoxon signed rank test will work well.
