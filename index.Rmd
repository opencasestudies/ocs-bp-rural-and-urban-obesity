...
For illustrative purposes we will show how we can use the `summarize()` and `group_by()` functions from `dplyr` to perform the Shapiro-Wilk test for all the subsets we are interested in.
...
Here, we see that all of the subsets of our data do not appear to be normally distributedexample of the test output for normal data - the p value is not < 0.05
