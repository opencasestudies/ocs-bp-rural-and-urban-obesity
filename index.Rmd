...
#### {.outline }
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "main_plot.png"))
```
...
knitr::include_graphics(here::here("www", "img","obesity_classes.jpeg"))
```
...
```{r,  echo=FALSE, out.width='100%'}
knitr::include_graphics(here::here("www", "img","BMIchart.jpeg"))
```
...
```{r,  echo=FALSE, out.width='100%'}
knitr::include_graphics(here::here("www", "img", "paper.png"))
```
...
5. Implementation of a [paired $t$-test](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5579465/){target="_blank"}  
6. Interpretation of a [paired $t$-test](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5579465/){target="_blank"}  
7. Implementation of a [Wilcoxon signed-rank test](http://www.biostathandbook.com/wilcoxonsignedrank.html){target="_blank"}
8.Interpretation of a [Wilcoxon signed-rank test](http://www.biostathandbook.com/wilcoxonsignedrank.html){target="_blank"}
...
According to this [report](https://www.who.int/nutrition/publications/bmi_asia_strategies.pdf){target="_blank"}:
...
##### [[source]](https://www.who.int/nutrition/publications/bmi_asia_strategies.pdf)
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img","first_page.png"))
```
...
```{r, echo=FALSE}
pdf_obesity <- pdftools::pdf_text(here("www", "data", "raw", "paper_supplement.pdf"))
```
...
```
Yes, this looks like data in the table we want. 
...
The `nchar.max` argument lets us define the maximal number of characters to show. 
Note `print(pdf_obesity[52])` would show the entire contents of this page of the table. 
...
knitr::include_graphics(here::here("www", "img","first_page_in_R.png"))
```
...
```{r, echo = FALSE }
knitr::include_graphics(here::here("www", "img","last_page.png"))
```
...
```{r, echo=FALSE}
save(rural_urban, file = here::here("www", "data", "imported", "imported_data.rda"))
```
...
An RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-rural-and-urban-obesity/master/data/imported) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-rural-and-urban-obesity/master/data/imported/impoted_data.rda). Download this file and then place it in your current working directory within a subdirectory called `imported` within a subdirectory called `data` to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
```{r, echo=FALSE}
save(rural_urban, file = here::here("www", "exercise", "dw_code1.rda"))
```
...
```{r dw_code1, exercise=TRUE, exercise.lines=5}
# Try this out! 
```
...
***
First let's try splitting our header-less data into columns based on spaces using the `str_split()` function, where we specify that we are splitting the data based on the pattern of a space (the space is included in quotes). Referring to [str_split() documentation](https://www.rdocumentation.org/packages/stringr/versions/1.4.0/topics/str_split), the `simplify` argument is set to `FALSE` by default, where a list of character vectors is returned. If it is set to `TRUE` as we will be doing here, a character matrix is returned.
...
However, we can also do the same thing in a single step, where we replace the `Women` dataset with the code we used to create that dataset.  So first, we select the "Women" and then we split the data by " Women" to create a new dataset all within the same chunk of code. Write this code yourself!
...
***
<details> <summary> Click here to see the code and the output </summary>
...
```{r, echo = FALSE, out.width="100%"}
knitr::include_graphics(here::here("www", "img","missing_pdf.png"))
```
...
We will also introduce the concept of the pipe operator `%>%`. 
The pipe operator is loaded with `tidyverse` (if you used `library(tidyverse)` which loads most tidyverse packages).
...
```{r}
Men <- Men %>% rename(Country = country_split) 
```
...
```{r, eval = TRUE}
#pull(BMI, National_BMI_1985) <- BMI %>% as.numeric #This will not work
BMI[["National_BMI_1985"]] <- BMI[["National_BMI_1985"]] %>% as.numeric
BMI[["Rural_BMI_1985"]] <- BMI[["Rural_BMI_1985"]] %>%  as.numeric
BMI[["Urban_BMI_1985"]] <- BMI[["Urban_BMI_1985"]] %>% as.numeric

BMI[["National_BMI_2017"]] <- BMI[["National_BMI_2017"]] %>%  as.numeric
BMI[["Rural_BMI_2017"]] <- BMI[["Rural_BMI_2017"]] %>% as.numeric
BMI[["Urban_BMI_2017"]] <- BMI[["Urban_BMI_2017"]] %>% as.numeric
```
...
For a more detailed description about this, please see the Data Visualization section of this [case study](https://www.opencasestudies.org/ocs-bp-opioid-rural-urban/#Rural_and_Urban_areas/#:~:text=%20the%20data%20was%20presented%20in%20a%20format%20that%20is%20called%20long%20format.){target="_blank"}.
...
This is based on the underscores (“_”) and creates two new columns. 
...
The `NA` in our code is for removing the middle part of the column (“BMI”). Thus, this part will not be used to create any new columns.
...
quiz(caption = "",
  question("A wider credible interval indicates higher accuracy of the estimate.",
    answer("True", message = "A wider credible interval suggests that the estimate is **less** accurate."),
    answer("False", correct = TRUE, message = "A wider credible interval suggests that the estimate is **less** accurate."),
    allow_retry = TRUE,
  ...
  question("Which one of the functions from the `stringr` package does not require a `pattern` argument?",
    answer("`str_split()`", message = "A pattern argument is needed when using `str_split()`. See its documentation [here](https://www.rdocumentation.org/packages/stringr/versions/1.4.0/topics/str_split)"),
    answer("`str_subset()`", message = "A pattern argument is needed when using `str_subset()`. See its documentation [here](https://www.rdocumentation.org/packages/stringr/versions/1.4.0/topics/str_subset) "),
    answer("`str_which()`", message = "A pattern argument is needed when using `str_which()`. See its documentation [here](https://www.rdocumentation.org/packages/stringr/versions/1.4.0/topics/str_subset) "),
    answer("`str_squish()`", correct = TRUE, message = "`str_squish()` is used to remove excess white-space, which does not need a `pattern` argument. See its documentation [here](https://www.rdocumentation.org/packages/stringr/versions/1.4.0/topics/str_trim) "),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
Suppose we have two datasets called `wide_format1` and `wide_format2`. Write some code to:\
Step 1. For `wide_format1`, change names of the columns to `ID`, `Year`, and `Length`. (Hint: You can do this by adding a new header or using the `rename()` function of the `dplyr` package.)\
Step 2. For `wide_format1`, add a new variable called `Adjusted` which has value = `Length` - 2. (Hint: Use the `mutate()` function from the `dplyr` package.)\
Step 3. Append `wide_format2` to `wide_format1` and call the new dataset `wide_format`. (Hint: Use `bind_rows()` function from the `dplyr` package.)\
Step 4. Arrange the data by `ID` in **descending** order. (Hint: Use the `arrange()` function from the `dplyr` package.)\
Step 5. Transform `wide_format` data into "long" format and call it "long_format". (Hint: Use `pivot_longer()` function from the `tidyr` package.) We want it to look like this:
...
An RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-rural-and-urban-obesity/tree/master/data/wrangled) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-rural-and-urban-obesity/master/data/wrangled/wrangled_data.rda). Download this file and then place it in your current working directory within a subdirectory called "wrangled" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily.
...
***
<details> <summary> Click here for additional information on using `ggplot2`.</summary>
...
**_What is the `ggplot()` function?_**
...
We could also make this same plot by piping the `norm_BMI_ex_data` data into the `ggplot()` function. Try writing the code yourself!
...
```{r de_code, exercise=TRUE, exercise.lines=5}
...
### **Exercise**
***
``` {r de_quiz, echo = FALSE}
quiz(caption = "",
  question("If the mean and the median of our data is similar, then the data is not skewed.",
    answer("False"),
    answer("True", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("What can Q-Q plots allow us to do? (Check all that apply)",
    answer("Compare the quantiles of the theoretical distribution and the distribution of our data.", correct = TRUE),
    answer("Gain visual evidence of if our data follows a normal distribution.", correct = TRUE),
    answer("Tell if our data is right- or - left skewed.", correct = TRUE),
    answer("Add a regression line that best fits our data.", message = "The line on a Q-Q plot is not a regression line."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
To check the normality of our data, we can use a histogram and/or a Q-Q plot. Suppose that we have dataset `mydata`, write some code that uses the `ggplot()` function to plot a histogram and a Q-Q plot. Does `mydata` seem to be normally distributed?
...
```{r de_exercise, exercise=TRUE, eval=FALSE}
# First we need mydata to be a tibble so that we can use the ggplot() function
...
```{r, echo = FALSE, out.width='100%'}
include_graphics(here::here("www", "img", "probability-line.svg"))
...
quiz(caption = "",
  question("What are the required assumptions of the data when performing a student t-test? (Check all that apply)",
    answer("Normality", correct = TRUE),
    answer("Large sample size", message = "t-test can be performed with small samples."),
    answer("The observations are independent and identically distributed", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("When performing a statistical test, what are our options if our data seems not to be normally distributed? (Check all that apply)",
    answer("Do a transformation of the data to make it more normally distributed.", correct = TRUE),
    answer("Use a nonparametric test.", correct = TRUE),
    answer("Perform a parametric test and see if the results are as expected.", message = "Parametric tests should not be performed when assumptions are violated."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Suppose we have a group of patients who have high blood pressure. Their baseline blood pressure measures were recorded on day 0. They then received a type of treatment for 30 days. Their blood pressure measures were recorded again on day 30. We want to test if their blood pressure measures changed (if the treatment was effective), what kind of t-test is the most appropriate? (Assume that the assumptions of a t-test are satisfied)",
    answer("Two-sample t-test", message = "Keep in mind that two measures are from the same patient."),
    answer("Welch's t-test (unequal variance t-test)", message = "Keep in mind that two measures are from the same patient."),
...
```{r, echo=FALSE}
png(here::here("www", "img", "main_plot.png"), height = 1500, width = 2300, res = 300)
cowplot::plot_grid(Means_plot, Diff_plot, labels = c("A", "B"))
dev.off()
```
...
<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSfpN4FN3KELqBNEgf2Atpi7Wy7Nqy2beSkFQINL7Y5sAMV5_w/viewform?embedded=true" width="640" height="891" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>
