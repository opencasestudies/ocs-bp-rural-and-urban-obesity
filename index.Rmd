...
Body mass index ([BMI](https://www.cdc.gov/bmi/adult-calculator/bmi-categories.html){target="_blank"}) is often used as a proxy for **adiposity** (the condition of having excess body fat) and is measured as an individual's weight in kilograms (kg) or pounds (lbs) divided by the individual's height in meters ($m^2$) squared.

There are different categories of weight defined using different BMI thresholds as defined by the [World Health Organization](http://www.euro.who.int/en/health-topics/disease-prevention/nutrition/a-healthy-lifestyle/body-mass-index-bmi){target="_blank"} (WHO), the [Centers for Disease Control and Prevention](https://www.cdc.gov/bmi/adult-calculator/bmi-categories.html){target="_blank"}, and the [National Institutes of Health](https://www.nhlbi.nih.gov/sites/default/files/media/docs/obesity-evidence-review.pdf){target="_blank"}. 
For example, these are the categories defined by the WHO: 

```{r,  echo=FALSE, out.width='100%'}
knitr::include_graphics(here::here("img","obesity_classes.jpeg"))
```

###### [[source](https://www.ncbi.nlm.nih.gov/books/NBK551660/figure/article-35266.image.f1/){target="_blank"}]

The following chart can help you identify your estimate of BMI:
```{r,  echo=FALSE, out.width='100%'}
knitr::include_graphics(here::here("img","BMIchart.jpeg"))
```

###### [[source](https://www.mayoclinic.org/diseases-conditions/obesity/in-depth/bmi-calculator/itt-20084938){target="_blank"}]
...
According to this [report](https://pubmed.ncbi.nlm.nih.gov/14726171/){target="_blank"}:
...
##### [[source]](https://pubmed.ncbi.nlm.nih.gov/14726171/)
...
While [gender](https://www.genderspectrum.org/resources){target="_blank"} and [sex](https://www.who.int/genomics/gender/en/index1.html){target="_blank"} are not actually binary, the data presented that is used in this analysis only contain data for groups of individuals described as men or women. 
...
If the package does not work for you, alternatively, an RDA file (stands for R data) of the data can be found [here](https://github.com/opencasestudies/ocs-bp-rural-and-urban-obesity/tree/master/data/imported) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-rural-and-urban-obesity/master/data/imported/imported_data.rda). Download this file and then place it in your current working directory within a subdirectory called `imported` within a subdirectory called `data` to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
The more we test data, the more likely we are to see a significant finding just by [random chance](https://online.stat.psu.edu/stat200/lesson/6/6.1){target="_blank"} even when the finding is in fact not real. It all comes down to probability.
...
```{r, echo = FALSE, out.width='100%'}
include_graphics(here::here("img", "probability-line.svg"))
...
Much of this explanation is described in this [lecture](https://online.stat.psu.edu/stat200/lesson/6/6.3){target="_blank"}.
...
[BMI](https://www.cdc.gov/bmi/adult-calculator/bmi-categories.html){target="_blank"}  
[Long and Wide Data Formats](https://r4ds.hadley.nz/data-tidy.html){target="_blank"}  
...
[Multiple Testing](https://online.stat.psu.edu/stat200/lesson/6/6.3){target="_blank"}    
