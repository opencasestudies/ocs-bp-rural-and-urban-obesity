...
Recently, an [article](https://www.nature.com/articles/s41586-019-1171-x.pdf){target="_blank"} published in [*Nature*](https://www.nature.com){target="_blank"} evaluated and compared the Body-Mass Index (BMI) of populations in rural and urban communities around the world: 
...
In this case study, we will walk you through importing data from a pdf, cleaning, wrangling and visualizing the data, and <b> comparing two groups </b> using well-established and commonly used packages, including `stringr`, `tidyr`, `dplyr`, `purrr`, and `ggplot2` from the [`tidyverse`](https://www.tidyverse.org/){target="_blank"}. 
...
We can check our data now using either `head()` from base R or `glimpse()` from the `tibble`  and `dplyr` package. (Yup, the same function is available from both packages)
...
Let's try to extract the country information by separating the country information from the sex information when the sex is female.
...
And then split this data based on the pattern `pattern = " Women"` (Note that the space before the word "Women" is included - so that it is not within the country data).
```{r}
country_split <- 
  Women %>%
...
```{r}
tibble::as_tibble(str_split(pull(unknown, rural_urban), " ",
                            simplify = TRUE))
...
Now that both of these tibbles have the same structure, we can combine our `Men` and `Women` data using the `full_join()` function of `dplyr`. 
...
For example, to plot the frequency distribution of a dataset we can use the `hist()` function to create a histogram.
...
Now, the <b>normal (or Gaussian) distribution</b> is a type of <b>theoretical continuous probability distribution</b> that is frequently used to approximate many naturally occurring distributions, such as that of BMI. 
We say that a random quantity $X$ (e.g. BMI) is normally distributed with average $m$ and standard deviation $s$ if the proportion of values in a given interval $(a,b)$ (e.g. BMI between 22 ans 26) can be computed using this mathematical formula: 

$$P(a < x < b) = \int_a^b \frac{1}{\sqrt{2\pi} s} e^{-\frac{1}{2}\big(\frac{x-m}{s}\big)^2}  $$

Translating this to our case study, if we say our data follow a normal distribution with mean of $m = 24$ and standard deviation of $s = 1$, then the probability of different BMI outcomes should be equally centered around the mean and would look something like this:
...
Ok so instead of using simulated data from a normal distribution, let's make a histogram of our BMI data
```{r}
BMI_long %>% 
  ggplot(aes(x = BMI)) + 
  geom_histogram() 
...
Interesting! The data looks like it has one mode, but like it is what we call <b>right-skewed</b> because the tail of the distribution is longer on the right side (in other words, it is a bit wider on that side). 
See [here](http://onlinestatbook.com/2/glossary/skew.html){target="_blank"} for more information about skewed distributions.
...
Another useful plot to create when exploring distributions is something called a "quantile-quantile" plot (or <b>Q-Q plots</b> for short). When we talk about a <b>quantile</b>, we are talking about dividing up the distribution of the data into roughly equal portions. For example, if you divide your data into 100 quantiles, you can think about this as percentiles, but you could also divide your data into 10 quantiles and this would be called deciles. 

<b>Why Q-Q plots?</b> This plot allows us to compare the quantiles of two distributions together: (1) quantiles from a known theoretical distribution (like the normal distribution) compared (2) quantiles the distribution of our data. This is useful because if the quantiles line up between the two distributions in the plot, then that is a visual piece of evidence that our data follow that theoretical distribution (like the normal distribution).  
...
Using the `stat_qq()` function of the `ggplot2` package, we can easily create a Q-Q plot for our data randomly sampled from a normal distribution ("sample") and compare it to the quantiles from a normal distribution ("theoretical")
```{r}
norm_BMI_ex_data %>%
...
The `stat_qq_line()` function is used to add a line (computes the slope an intercept) on the plot. 
Not surprisingly, we see that the quantiles are fairly similar between the observed and theoretical data.
We see that the points mostly fall on the line, however there are some points that are a bit further from the line as we get to the extreme quantiles. 
Notice that the sample quantiles (y-axis are in a fairly similar range to our real BMI data quantiles. 
So values that are below 22, for example, are represented as the points below 22 on the y-axis. 
As expected we see that about half the points are below our mean of 24.

If we were to use different data that had a range of different values our y-axis would shift according to the range of values. 
For example, the `Orange` data (within the installation of R) includes data about the circumference of orange trees in millimeters. 
Here, we can see that the quantiles are quite different but reflect the range of orange tree circumferences.
```{r}
...
We can can also see that our data is right- or positive-skewed, because the extreme values are above the line that indicates a perfect match between the theoretical and observed data.
...
It turns out if you want to ask questions like "Is there statistically significant differnce in the average BMI estimates from women in 1985 compared to women in 2017?", we will need to use some of type of statistical test. 
...
So, if we want to use those powerful statistical tests that relay on this assumption of "normality", then we need to check that assumption before using those tests. 

One way to do that is to use Q-Q plots. 
Another way to do that is to use the [Shapiro-Wilk test for normality](http://www.statistics4u.info/fundstat_eng/ee_shapiro_wilk_test.html){target="_blank"} when the Q-Q plot is a bit unclear. Many statisticians however would conclude from the Q-Q plots that normality appears to be violated in our data. 

### Shapiro-Wilk test

Here, we are using the `summarize()` and `group_by()` functions from `dplyr` to perform the Shapiro-Wilk test for all the subsets we are interested in. In this case the `$` is used to grab the `p.value` from the `shapirotest()` output.
...
We can see that all of the subsets of our data do not appear to be normally distributed because the $p$-value is less than 0.05 for all subsets. 
This is not suprising as the Q-Q plots suggested the same thing. 

## Data Analysis

In the previous section, we hinted at the existance of statistical tests to be able to assess if there is a statistically significant difference in the average BMI estimates between two groups (e.g. women in and 2017). 
For example, one test is the [Student's $t$-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test){target="_blank"} which can be used to determine if [two sample means are significantly different](http://onlinestatbook.com/2/estimation/difference_means.html){target="_blank"}.
However, this test depends on certain assumption of our data. 

In this section, we are going to talk about this test and other tests that do not have assumptions about the data. 
...
In hypothesis testing, we are interested in comparing two different hypotheses: a "null" hypothesis (can be thought of like a baseline e.g. the means between two groups are the same) compared to an "alternative" hypothesis (e.g. the means between two groups are different), where we are going to ask if there is enough in our data to reject the null hypothesis. 
Let's try to formalize this a bit. 

We want to perform what is called <b>two means hypothesis tests</b> using a statistical test (e.g. a Student's $t$-test). 
To do this, we define what is called the <b>null hypothesis</b> that there is no difference in the two means:

$$ H_0: \mu_1 = \mu_2 $$
where $\mu_1$ and $\mu_2$ are the true unknown means of group 1 and group 2, respectively. 
In contrast, we also define an <b>alternative hypothesis</b> that there is a difference between the two means: 

$$ H_a: \mu_1 \neq \mu_2 $$

The way it works is that we assume the null hypothesis is true and we use our data to help us identify if there is enough evidence to _reject the null hypothesis_. 
This is similar to the idea of assuming that individuals are not guilt until proven otherwise.
If there is not enough evidence in the data, then we say we "fail to reject the null hypothesis", or another way of stating it is that there was not enough evidence in the data to "reject the null hypothesis". 

You might be asking, "but we do not know what the true group means are?". 
That is true. 
To address this, we estimate the averages based on our data that we have.
For example, in our data, we can estimate difference in the average BMI estimates for women in rural and urban regions in the two different years. 

```{r}
BMI_long %>% 
  filter(Sex == "Women", Region != "National") %>% 
...
BMI_long %>% 
  ...
  group_by(Region, Year) %>%
  summarize(avg_bmi = mean(BMI, na.rm = TRUE)) %>% 
  mutate(diff = avg_bmi - lag(avg_bmi)) %>% 
  drop_na() %>%
  pull(diff)
```

Next, we can use a statistical test (e.g. a Student's $t$-test) to assess if these differences are statistically meaningful or if these differences could be found due to random change. 
...
<b>Parametric tests</b> are based on assumptions about the distribution of the data, while <b>nonparametric tests</b> do not rely on this assumption. 
It is called "parametric" because aspects about the distribution of the data, like the mean, are called <b>parameters</b> of a mathematical function when we describe a population. 
In parametric tests, we estimate the parameters of the true population of interest using a sample of that population. 
These estimates are called <b>statistics</b>. 
...
Often when comparing two groups we might perform a two-sample $t$-test to determine if the means of each group is different. 
The two-sample $t$-test however, relies on several <u><b>assumptions</b></u>:

1) The data for both groups is [normally distributed](http://onlinestatbook.com/2/introduction/distributions.html){target="_blank"}
2) The [variance](https://stattrek.com/statistics/dictionary.aspx?definition=variance){target="_blank"} of both groups is similar
3) The number of observations is similar for both groups - thus they are [balanced](https://www.statisticshowto.datasciencecentral.com/balanced-and-unbalanced-designs/){target="_blank"}
4) The observations are [independent](https://www.stat.cmu.edu/~cshalizi/36-220/lecture-5.pdf){target="_blank"} (meaning that observations do not influence each other)
...
1) [Transformation](https://www.statisticshowto.datasciencecentral.com/transformation-statistics/){target="_blank"} of the data to make it more normally distributed
2) [Welch's $t$-test](https://www.statisticshowto.datasciencecentral.com/welchs-test-for-unequal-variances/){target="_blank"} also call the unequal variance $t$-test we may need to modify the way we perform the [$t$-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test){target="_blank"} to account for the difference in the variance in the two groups
3) [Permutation/resampling methods](https://jhu-advdatasci.github.io/2019/lectures/21-resampling-techniques.html){target="_blank"} to deal with violations of normality or imbalance.
...
If it were violated, we would want to consider using permutation methods which are also a good option for violations of normality. To learn more about these methods see [here](https://jhu-advdatasci.github.io/2019/lectures/21-resampling-techniques.html){target="_blank"}.
...
There are multiple nonparametric options to consider when the assumptions of the [$t$-test] are violated. The [Wilcoxon signed rank test](http://www.biostathandbook.com/wilcoxonsignedrank.html){target="_blank"} (for paired data - the alternative is [Wilcoxon rank sum test](http://sphweb.bumc.bu.edu/otlt/mph-modules/bs/bs704_nonparametric/BS704_Nonparametric4.html){target="_blank"} (also called the Mann-Whitney U test) for independent samples) and the [two-sample Kolmogorov-Smirnov test](https://www.itl.nist.gov/div898/software/dataplot/refman1/auxillar/ks2samp.htm){target="_blank"} (KS) both do not assume normality (has both paired and unpaired methods). Thus these tests should be considered when the data of either groups does not appear to be normally distributed and particularly when the number of samples is low.
...
Now let's save our plotas a png using the `png()` function:

```{r}
png(here::here("img", "main_plot.png"), height = 1500, width = 2000, res = 300)
Means_plot +labs(tag = 'A') + Diff_plot+ labs(tag = 'B') +
  plot_layout(guides = 'collect', ncol = 2)
dev.off()
