...
An [article](https://www.nature.com/articles/s41586-019-1171-x.pdf) published in Nature evaluated and compared the Body-Mass Index (BMI) number of populations in rural and urban communities around the world. 
...
First let's download the data so we can take a look at the original data:
```{r}
library(XML)
library(here)
...
We also have a lot of white-space... let's get rid of the excess white spaces.
...
So close! Notice that there are `na*` values that have shifted the subsequent values within the columns because typically there is a space between the BMI and the credible intervals. Here we can see this data in our original pdf:
...
First let's see how the results look if we were to ignore the fact that our data does not appear to be normally distributed.

```{r}
for_hist <-BMI %>% select(-matches("CI|ex|Country"))# to remove columns with names containing CI, such as "Urban_BMI_1985_CI or "Country"
rownames(for_hist)<- paste(BMI$Country, BMI$Sex, sep = "_")
lapply(for_hist, hist)
```
