...
This is because it's not useful to construct a distribution that assigns a probablity to each possible outcome (e.g. a BMI of 22.11, 22.12, etc) with such high precision. In this case, we would have to assign a very small probability to every possible BMI outcome. 

Now, the <b>normal (or Gaussian) distribution</b> is a type of <b>theortical continuous probability distribution</b> that is frequently used to approximate many naturally occurring distributions, such as that of BMI. 
We say that a random quantity $X$ (e.g. BMI) is normally distributed with average $m$ and standard deviation $s$ if the proportion of values in a given interval $(a,b)$ can be computed using this mathematical formula: 
...
Alternatively to using base R, we can plot this using the `geom_hist()` function of the `ggplot2` R package. 
