...
#Was global BMI lower in Rural or Urban areas in 1985 based on  the t value?
```
...
```
There is a significant difference across time for both regions.
...
BMI_diff_long <- BMI_numeric %>% select(Country: National_difference) %>% 
...
#Let's see who:
BMI_diff_long %>% arrange(Difference)
 ...
 scale_fill_manual(values=c("dodgerblue", "orchid2"))+
  ...
  geom_text(data=subset(BMI_long
                         %>%filter(Sex %in% c("Men", "Women"), 
                            ...
                            aes(x =Sex, y =BMI,label=Country), color = "black") + 
  # This will add USA labels to the USA points
  theme_linedraw()+
