...
###### [BMI Chart](https://www.vertex42.com/ExcelTemplates/bmi-chart.html) created by Vertex42.com. Used with permission.
...
```{r}
rural_urban <-stringr::str_subset(pattern ="Age-standardised mean BMI", string =pdf_obesity)
...
Great, now our rural_urban object 400 lines instead of 461, like it did before when it contained the redudant header information.
...
Thus the even rows - the rows with the data for women have a remainder of zero.
...
# note that there will be a space in between because it is already in front of the first BMI value
```
...
Women<-stringr::str_replace_all(string = Women, pattern = "na\\*", replacement = "na\\* na\\*") 
Women[20:30]
```
...
head(BMI_numeric)
# In this case our numeric data is class dbl - which means double-precision floating point numbers
# The alternative numeric option is the integar class - but we would of course like to retain our decimal values
```
...
In our case we will relace the class_BMI column with two new columns.
...
In order to apply a statistical test to compare the means, one of the first things  to do is to plot the frequency of the different possible values. This is called a <b>distribution</b>. To do this we can use the `hist()` function to create a histogram.
...
OK, so the data looks like it is what we call <b>right-skewed</b> because the tail of the distribution is longer on the right side (in other words, it is a bit wider on that side), but it looks fairly normally distributed.
...
We can use statistical tests like the stud t-test to determine if the means are significantly different.

In these <b> two means hypothesis tests </b> we test the <null hypothesis that there is no difference in the two means:
...
3) The number of observations is similar for both groups - thus they are <b> balanced </b>
...
If it were violated, we would want to consider using permutation methods. To learn more about these methods see [here](https://jhu-advdatasci.github.io/2019/lectures/21-resampling-techniques.html).
...
We can see that the number of obersvations for each possible group of interest is the same.
...
#pull() will allow us to take the BMI column of the resulting data when when filter BMI_long

mood.test(dplyr::pull(filter(BMI_long, Sex == "Women", Year == "2017", Region == "Rural"), BMI), 
          dplyr::pull(filter(BMI_long, Sex == "Women", Year == "2017", Region == "Urban"), BMI))
...
# the t value was negative thus the BMI was lower in 1985 (the 1st group) compared to 2017 (the 2nd group)

# Question opportunity: Was global BMI lower in Rural or Urban areas in 1985 based on looking at the t value?
```
...
It's important to note that utlimately we wanted to test 4 different tests:
...
So we see a significant difference in the means of the groups after mutiple testing correction for our different tests.

Again, it would be reasonalble to use the t-test because it is robust to deviations in normality when samples are relatively large. 
We can see that we obtained the same results regarless of the test that we used.
However, if sample sizes are small (generally speaking <25 per group), then these nonparametric options are useful to know.

The D values in the output of our KS tests, show the magnitude of distancein the differnce between the distributions of the groups tested.
...
BMI_long %>% 
  ...
  geom_boxplot() +
  geom_jitter(width = .3) # the width determines how wide the points are plotted
...
BMI_long %>% 
  filter(Sex == "Women", Year %in% c("1985", "2017"), Region %in% c("Rural", "Urban")) %>%
  # filtering this way allows us to keep data from both years
  ggplot(aes(x = Year, y = BMI)) +
  geom_boxplot() +
  geom_jitter(width = .3) +
...
# we would get national data too if we did not include `Region %in% c("Rural", "Urban"))`
...
# thus there are some women in some countries that especially need help while others are actually showing a loss of BMI
...
# ok so some of the data for Men looks fairly normal but not the data for Women

# Now we will check the variance
BMI_diff_long %>% 
  group_by(Type, Sex) %>% 
  summarize(shapiro_test = shapiro.test(Difference)$p.value)


mood.test(pull(filter(BMI_diff_long, Sex == "Women", Type == "Rural_difference"), Difference), 
          pull(filter(BMI_diff_long, Sex == "Women", Type == "Urban_difference"), Difference))
#reject the null - conclude that variance is not equal

mood.test(pull(filter(BMI_diff_long, Sex == "Men", Type == "Rural_difference"), Difference), 
          pull(filter(BMI_diff_long, Sex == "Men", Type == "Urban_difference"), Difference))
#accept the null - conclude that variance is equal

mood.test(pull(filter(BMI_diff_long, Sex == "Women", Type == "Rural_difference"), Difference), 
          pull(filter(BMI_diff_long, Sex == "Men", Type == "Rural_difference"), Difference))
#reject the null - conclude that variance is not equal

ks.test(pull(filter(BMI_diff_long, Sex == "Women",  Type == "Rural_difference"), Difference), 
        pull(filter(BMI_diff_long, Sex == "Women",  Type == "Urban_difference"), Difference),
        paired = TRUE) 

# this is again still paired because each value of "Rural_difference" and "Urban_difference" correspond to the same country

ks.test(pull(filter(BMI_diff_long, Sex == "Men",  Type == "Rural_difference"), Difference), 
        pull(filter(BMI_diff_long, Sex == "Men",  Type == "Urban_difference"), Difference),
        paired = TRUE)

ks.test(pull(filter(BMI_diff_long, Sex == "Men",  Type == "Rural_difference"), Difference), 
        pull(filter(BMI_diff_long, Sex == "Women",  Type == "Rural_difference"), Difference),
        paired = TRUE)

t.test(pull(filter(BMI_diff_long, Sex == "Women",  Type == "Rural_difference"), Difference), 
        pull(filter(BMI_diff_long, Sex == "Women",  Type == "Urban_difference"), Difference),
        paired = TRUE, var.equal = FALSE)

t.test(pull(filter(BMI_diff_long, Sex == "Men",  Type == "Rural_difference"), Difference), 
        pull(filter(BMI_diff_long, Sex == "Men",  Type == "Urban_difference"), Difference),
...
There is a larger difference in the Rural communities for both Men and Women. The Rural difference is significantly different between men and women according to the KS test, but but not according to the t-test.
...
```{r}
BMI_diff_long %>% filter(Country == "USA")
...
```{r}
BMI_diff_long %>%filter(Difference>4.9) %>%arrange(-Difference)
...
```{r}
BMI_long %>% filter(Country %in% pull(filter(BMI_diff_long, Difference>4.9), Country), 
                    Sex %in% pull(filter(BMI_diff_long, Difference>4.9), Sex),
                    Year =="2017") %>%
            arrange(-BMI)

#Here is the data just for the changes in BMI in Egypt
filter(BMI_diff_long, Country == "Egypt") %>% arrange(-Difference)
...
The data suggests that rural women in Egypt and other countries may especially benefit from dietary resources and our interventions and programs to assist with weight managment.
...
Means_plot <-Means_plot+ geom_text(data = obesity_text,label=pull(obesity_text,label), color = "red", aes( fontface ="bold.italic", size = 13))
