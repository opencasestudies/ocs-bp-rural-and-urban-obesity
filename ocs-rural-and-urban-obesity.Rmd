...
Here we can see that these countries have the largest differences in BMI- the largest differences are all for Women in rural areas. 

```{r}
BMI_diff_long %>%filter(Difference>4.8)
```

However it is important to see what the start and end point mean BMI values are for these countries. 
It could be that the average was underweight in 1985... let's take a look.

```{r}
BMI_long %>% filter(Country %in% pull(filter(BMI_diff_long, Difference>4.8), Country), 
                    Sex %in% pull(filter(BMI_diff_long, Difference>4.8), Sex))

```

The data suggests that rural women in Egypt and Saint Lucia may especially benefit from dietary resources and our interventions and programs to assist with weight managment.
