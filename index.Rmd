...
Remeber the even rows are the rows with the BMI data for women.
...
To start, we could change these values to be numeric with 6 lines of code like this, where the double brackets`[[]]` and quotes are used to select the specific varaible within the `BMI` tibble. In this case we need to use the brackets instead of `pull()` becuase we can't start a command with `pull()` to reassign values in a variable.
