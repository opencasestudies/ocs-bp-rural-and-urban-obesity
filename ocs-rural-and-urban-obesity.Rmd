...
## Motivation
[BMI](https://www.cdc.gov/healthyweight/assessing/bmi/adult_bmi/index.html) is used as a proxy for <b>adiposity</b> and is measured as an individual's weight in kilograms divided by the individual's height in meters squared. The different categories of weight are defined with the following BMI cutoffs as defined by the [World Health Organization](http://www.euro.who.int/en/health-topics/disease-prevention/nutrition/a-healthy-lifestyle/body-mass-index-bmi), the [Centers for Disease Control and Prevention](https://www.cdc.gov/obesity/adult/defining.html), and the [National Institutes of Health](https://www.nhlbi.nih.gov/sites/default/files/media/docs/obesity-evidence-review.pdf):

![](https://www.ncbi.nlm.nih.gov/books/NBK535456/bin/bmi_WHO.jpg)

###### [BMI table source](https://www.ncbi.nlm.nih.gov/books/NBK535456/bin/bmi_WHO.jpg)

The following chart can help you identify your BMI estimate:
![](http://content.everydayhealth.com/mayodiet/images/BMI-chart.jpg)

###### [BMI Chart source](http://diet.mayoclinic.org/diet/eat/what-is-your-bmi?xid=nl_MayoClinicDiet_20160426) 

Higher BMI (>30) is associated with [higher rates of all-causes of mortality](https://www.nhlbi.nih.gov/sites/default/files/media/docs/obesity-evidence-review.pdf), as well as increased rates of type 2 diabetes, cancer, heart disease, and stroke.
...
####
This article challenged the widely-held view that increased urbanization was one of the major reasons for increased global obesity rates. This view came about because many countries around the world have shown increased urbanization levels in parallel with increased obesity rates. However this study demonstrated that this might not be the case; and that in fact for most regions around the world, BMI measurements are increasing in rural populations just as much, if not more so, than urban populations. Furthermore, this study suggested that obesity has particularly increased in female populations in rural regions. "We noted a persistently higher rural BMI, especially for women" This case study will evaluate the data reported in this article to explore regional and gender specific differences in the obesity rates around the world in 1985 and 2017. Most importantly we will test if there is a difference in obesity rates between rural and urban communities and if there has been a change in obesity over time for these regions, particularly for females. To do this we will test if there is a difference between the means of the BMI measurements for each group.
...
# but wasn't able to find it, or it isn't an Array"

# Errors like this can be common with PDFs
# We can take a look at the data to see if it still looks as expected
```
...
```{r, eval = FALSE}
pdf_obesity[55] 
# this looks like data in a the table we want
```
...
# "Age-standardised mean BMI" is part of the header in the table on every page.
...
# There are 10 pages worth of elements in our rural_urban object
...
First let's see how many lines include "women" or "men". We can do this by first identifying the lines that contain these patterns and then check the length of the resulting vector for the line numbers that contain these patterns.

```{r}
stringr::str_which(string = rural_urban, pattern = "Women|Men")
length(stringr::str_which(string = rural_urban, pattern = "Women|Men"))
# Here the "|" indicates that we are looking for either  "Women" or "Men"
```
...
```{r}
rural_urban <- str_subset(string = rural_urban, pattern = "Women|Men")
```
...
```{r}
(str_split(string = rural_urban, pattern = " ", simplify = TRUE))[1:10,] 
# Here we will take a look at just the first 10 lines using the [1:10,]
```
...
Women <- str_subset(string = rural_urban, pattern = "Women") 
...
#we can do the same thing with two pipes:
# first we select the "Women" data and then we split the data by " Women"
# finally we assign the output of both steps to the object country_split

country_split <- str_subset(string = rural_urban, pattern = "Women") %>%
  stringr::str_split(pattern= " Women") %>%
  unlist()
...
```{r}
Women_BMI <-tibble(country_split) %>% 
...
```{r}
Women <-glue::glue("Women{Women_BMI$country_split}")
head(Women)
# here we will paste " Women" in fromt of the Women_BMI data
...
# In order to use the functions within the stringr package, 
# our Men data needs to be of character class
Men<-as.character(Men$rural_urban) 
# This function allows us to change the factor data 
# within Men$rural_urban to the character class
# We are also changing the structure so that the data 
# is not within a vector called rural_urban
Men<-stringr::str_replace_all(string = Men, 
                              pattern = "na\\*", 
                              replacement = "na\\* na\\*") 
...
Women<-stringr::str_replace_all(string = Women, 
                                pattern = "na\\*", 
                                replacement = "na\\* na\\*") 
Women[20:30]
```
...
```{r}
#header
new_header <-c("Sex","National_BMI_1985", "National_BMI_1985_CI", "Rural_BMI_1985", 
...
# We have also reassigned Men to the data object 
# which has the country variable renamed
```
...
In other words, if we want to make plots about National BMI in 1985 then we would need our values to be numeric. Looking at our BMI data using `str()`, we can see the type of data for each of our variables listed just after variable name and the ":" colon. Looks like none of our BMI data is actually numeric, but of the class character. Let's change that now.
...
#  And if we did this we would see these variables 
#  show as "num" which stands for numeric
str(BMI)
```

Or we can use a more automated way with the `map()` function of the `purrr` package:
```{r}
BMI_numeric <- as_tibble(purrr::map(
  (BMI %>% select(-matches("CI|Sex|Country"))), as.numeric)) 
# The map function of the purrr package allows us to apply the 
# as.numeric() function to all the selected columns of BMI 
...
head(BMI_numeric)
# In this case our numeric data is class dbl
# This means double-precision floating point numbers (with decimals)
# The alternative numeric option is the integar class
```
...
Basically what the long format will have more rows and fewer columns.
...
# The key indicates the new name of the column that will now contain 
# information about the values of the columns we will put together
# The value is the name of the column for the values for 
# all the columns that are being put together
# The ":" can be used to indentify the start and end of the column names
# that you are putting together
# The factor_key paramater determines if the newly created key column 
# should be evaluated as a factor or not
head(BMI_long)
```
...
# We will separate the class_BMI column of the BMI_long tibble
...
```{r, echo = FALSE}
save(BMI_numeric, BMI_long, file = here("Wrangled_data.rda"))
...
```
OK, so our means are fairly similar, but we can see some differences. 
The question is if those differences are meaningful. 
...
In order to apply a statistical test to compare the means, one of the first things  to do is to plot the frequency of the different possible values. This is called a <b>distribution</b>. See [here](http://onlinestatbook.com/2/introduction/distributions.html) for more informations about distributions. To plot the distribution of a dataset we can use the `hist()` function to create a histogram.

If the data were what we call <b>normally distributed</b>  or  followed a <b> Gaussian distribution </b> then the distribution should be equally centered around the mean and would look something like this:

```{r}
norm_BMI_ex_data <- tibble(norm_data =rnorm(n = 200, mean = 24))
# The rnorm() function allows us to make a vector of normally distributed data
# centered around the mean of 24
head(norm_BMI_ex_data)
hist(norm_BMI_ex_data$norm_data)
```

Alternatively we can plot this using the `geom_hist()` function of the `ggplot2` package. The [ggplot2](https://ggplot2.tidyverse.org/) package creates plots by using layers. Notice in the following code how there is a plus sign between the `ggplot()` function and the `geom_histogram()` function. With ggplot2 we select what data we would like to plot using the first function (`ggplot()`) and then we add on additional layers of complexity (these layers can even involve different data). In this case the `geom_histrogram()` function initiates the plotting of a histogram with the data that was identified in the `ggplot()` function. We will see later how we can add many layers to plots with `ggplot2`. For additional information on using ggplot2, see this [case study](https://opencasestudies.github.io/ocs-healthexpenditure/ocs-healthexpenditure.html).
...
# We are using the column called norm_data within norm_BMI_ex_data
# With ggplot2 we must first specify the data with ggplot() 
...
OK, so the data looks like it is what we call <b>right-skewed</b> because the tail of the distribution is longer on the right side (in other words, it is a bit wider on that side), but it looks fairly normally distributed. See [here](http://onlinestatbook.com/2/glossary/skew.html) for more information about skewed distributions.
...
OK, so some of these plots look pretty similar to our normal distribution plot, like the Women 2017 Urban plot (although it is right-skewed).
However some of the other plots, like the Men 2017 Urban plot, look very different. These plots show what is called a [bimodal distribution](http://onlinestatbook.com/2/introduction/distributions.html), meaning that there appear to be two peaks, or in other words two different values (and values near each of these) that are the most frequent.
...
With the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test), we perform what is called <b> two means hypothesis tests </b>.
...
Thus like in law, we accept this null hypothesis, unless we have enough evidence to suggest that we should reject it - similarly to the idea that we by default assume that indivudals are not guilty until proven otherwise. If we reject the null hypothesis, we then accept an <b> alternative hypothesis </b>, this can vary for different statistical tests, but in the case of the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) we evaluate if the two means are not equal:
...
Parametric tests are based on assumptions about the distribution of the data, while Nonparametric tests do not rely on this assumption. It is called "parametric" because aspects about the distribution of the data like the mean aer called <b>parameters</b> when we describe a population. In parametric tests we estimate the parameters of the true population of interest using a sample of that population. These estiamtes are called <b>statistics</b>. See [here](https://www.mayo.edu/research/documents/parametric-and-nonparametric-demystifying-the-terms/doc-20408960) for more information about the difference between these two classes of tests.

### Parametric two sample mean tests

Often when comparing two groups we might perform a two sample [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) to determine if the means of each group is different. The two sample [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) however, relies on several <u><b>assumptions</b></u>:
...
4) The observations are [independent](https://www.stat.cmu.edu/~cshalizi/36-220/lecture-5.pdf) (meaning that observations do not influence eachother)

If these assumptions are violated, this doesn't necessarily mean we can't perform a [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test). It just means we may need to consider the following options:
...
```{r}
table(BMI_long$Sex, BMI_long$Region, BMI_long$Year)
```

We can see that the number of observations for each possible group of interest is the same.

The [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) is also fairly robust to non-normality if the data is relatively large, due to what is called the [central limit theorem](https://www.analyticsvidhya.com/blog/2019/05/statistics-101-introduction-central-limit-theorem/).
...
The alternative hypthesis is that the mean of the differences is not equal to zero
...
The data appears to be more similar to the normal distribution, although not quite. Again, our sample size of 200 is quite large and the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) is generally quite robust to violations of normality with large n, thus the modified [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) to account for unequal variance might be a good option using the log normalized data, as it is at least more normally distributed.

Let's see the results of the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) with the transformed data:
```{r}
...
```
There is a significant difference across time for both regions, as we saw with the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test). There is also a significant difference by region for each year. However, the p-values are a bit larger for the KS test results than we saw with the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test).
...
As an example, let's take a look at some simulated data.

```{r}
#a<-r
```

One way to do this is using the [Bonferroni method](http://mathworld.wolfram.com/BonferroniCorrection.html. 
...
Again, it would be reasonable to use the [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) because it is robust to deviations in normality when samples are relatively large. 
...
mood.test(pull(filter(BMI_diff_long, 
          ...
          pull(filter(BMI_diff_long, 
                      Sex == "Men", 
                      Type == "Urban_difference"), Difference))
#accept the null - conclude that variance is equal
...
1) We can still perform a [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) if our n is large
2) We can transform the data before performing a [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test)
...
4) We can perform a [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) with resampling methods (which should be especially considered when the groups are imbalanced)

We learned that if our groups do not have equal variance, that we need to use a modified [t-test](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test) to account for this or the correct nonparametric test that does not rely on this assumption (Two-sample Kolmogorov-Smirnov test).
