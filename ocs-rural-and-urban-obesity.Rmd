...
[purrr](https://purrr.tidyverse.org/)      | to perform functions on all colums of a tibble
...
[ggrepel](https://cran.r-project.org/web/packages/ggrepel/vignettes/ggrepel.html)    | to allow labels in figrues not to overlap
___
...
It is generally useful to get the data in what is called <b>long</b> format for other analyses, and particularlly for plotting. 
...
If we were to use different data that had a range of different values our y-axis would shift according to the range of values. For example the Orange data within the installation of R includes data about the circumfrance of orange trees in millimeters. Here we can see that the quantiles are quite different but reflect the range of orange tree circumfrances.
```{r}
...
Finally, some statisticians also use the Shapiro-Wilk test for normality when the qqplot is a bit unclear. Many statisticians however would conclude from the QQ plots that normality has been violated.

For illustrative purposes we will show how we can  use the dplyr summarize() and group_by() functions to perform the shapiro test for all the subsets we are interested in.
...
We see that all the data looks like it is not normally distributed.
...
The t test is also fairly robust to non-normality if the data is relatively large, and we have an n of 200, which should be sufficent but let's investigate the nonparametric tests further. 
...
mood.test(women_2017$Urban, women_2017$Rural)

mood.test(women_1985$Urban, women_1985$Rural)
...
Importantly the Kolmogorov-Smirnov test does not assume normality or equal variance, while the Wilcoxon signed rank test does assume equal variance. Here is how you would perform these tests. However in our case, becuase the variance is not equal between our groups of interest, the Kolmogorov-Smirnov test would be more appropriate.
...
```{r}
ks.test(women_1985$Urban, women_2017$Urban, paired = TRUE)
ks.test(women_1985$Rural, women_1985$Rural, paired = TRUE)
...
women_1985$country <- country$country_split 
# Let's put the type of BMI in long format again, but not inlcude the country in long format
women_1985_long<-gather(women_1985, key = Type, value = BMI, Urban:Rural) 
head(women_1985_long)

# Let's add the country names
women_2017$country <- country$country_split 
# Let's put the type of BMI in long format again, but not inlcude the country in long format
women_2017_long<-gather(women_2017, key = Type, value = BMI, Urban:Rural)
head(women_2017_long)

ggplot(women_1985_long, aes(x = Type, y =BMI, col = Type)) + geom_boxplot() + geom_label(aes(label = country))  
```
If we include all country names this is a bit too much... so perhaps we should focus on just the extreme BMI values. We will also use the `ggrepel` package to have our labels not overlap eachother.
```{r}
ggplot(women_1985_long, aes(x = Type, y =BMI, col = Type)) + geom_boxplot(outlier.shape = NA)+geom_text(data=subset(women_1985_long, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country)) + geom_jitter(data=subset(women_1985_long, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country), width = .02) + ggrepel::geom_text_repel(data=subset(women_1985_long, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country))
```
...
women_diff <-women_2017 -women_1985
 ...
 # Let's add the country names
women_diff$country <- country$country_split
# Let's put the type of BMI in long format again, but not inlcude the country in long format
women_diff_long<-gather(women_diff, key = Type, value = Difference, Urban:Rural) 
