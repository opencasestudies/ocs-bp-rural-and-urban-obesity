...
women_1985<-gather(women_1985, key = Type, value = BMI, Urban:Rural) # Let's put the type of BMI in long format again, but not inlcude the country in long format
head(women_1985)

women_2017$country <- country$country_split # Let's add the country names
women_2017<-gather(women_2017, key = Type, value = BMI, Urban:Rural) # Let's put the type of BMI in long format again, but not inlcude the country in long format
head(women_2017)

ggplot(women_1985, aes(x = Type, y =BMI, col = Type)) + geom_boxplot() + geom_label(aes(label = country))  
```
If we include all country names this is a bit too much... so perhaps we should focus on just the extreme BMI values:
```{r}
library(ggrepel)
ggplot(women_1985, aes(x = Type, y =BMI, col = Type)) + geom_boxplot(outlier.shape = NA)+geom_text(data=subset(women_1985, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country)) + geom_jitter(data=subset(women_1985, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country), width = .02) + geom_text_repel(data=subset(women_1985, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country))
```

And let's fill the boxplots with color and outline in black:
```{r}
ggplot(women_1985, aes(x = Type, y =BMI, col = Type)) + geom_boxplot(outlier.shape = NA, aes(fill = Type), color = "black") + geom_jitter(data=subset(women_1985, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country), width = .02) + geom_text_repel(data=subset(women_1985, BMI>31 | BMI<19 | country == "United States of America"), aes(x =Type, y =BMI,label=country))

ggplot(women_2017, aes(x = Type, y =BMI, col = Type)) + geom_boxplot(outlier.shape = NA, aes(fill = Type), color = "black") + geom_jitter(data=subset(women_1985, BMI>33 | BMI<20 | country == "United States of America"), aes(x =Type, y =BMI,label=country), width = .02) + geom_text_repel(data=subset(women_1985, BMI>31 | BMI<20 | country == "United States of America"), aes(x =Type, y =BMI,label=country))
```
...
pdf(file = here("mainplot.pdf"), width = 8, height = 6)
