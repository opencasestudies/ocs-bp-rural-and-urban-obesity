...
Body mass index ([BMI](https://www.cdc.gov/healthyweight/assessing/bmi/adult_bmi/index.html)){target="_blank"} is often used as a proxy for **adiposity** and is measured as an individual's weight in kilograms (kg) or pounds (lbs) divided by the individual's height in meters ($m^2$) squared.
...
Here, we are using the `summarize()` and `group_by()` functions from `dplyr` to perform the Shapiro-Wilk test for all the subsets we are interested in.
